...
struct var::VariantType
{
    ...
    static void voidWriteToStream (const ValueUnion&, OutputStream& output) { output.writeCompressedInt (0); }

    constexpr explicit VariantType (VoidTag) noexcept
        : isVoid (true), isComparable (true), equals (voidEquals), writeToStream (voidWriteToStream)
    {}
    ...
    static bool
    doubleEquals (const ValueUnion& data, const ValueUnion& otherData, const VariantType& otherType) noexcept
    {
    ...
    {
        return unalignedPointerCast<const String*> (data.stringValue);
    }
    ...
    {
        return getString (data)->getIntValue () != 0 || getString (data)->trim ().equalsIgnoreCase ("true") ||
    ...
    static bool
    stringEquals (const ValueUnion& data, const ValueUnion& otherData, const VariantType& otherType) noexcept
    {
    ...
    static bool
    objectEquals (const ValueUnion& data, const ValueUnion& otherData, const VariantType& otherType) noexcept
    {
    ...
    static void bufferCleanUp (ValueUnion& data) noexcept
    {
        if (data.objectValue != nullptr)
            data.objectValue->decReferenceCount ();
        ;
    ...
    static String bufferToString (const ValueUnion& data)
    {
        return "Object 0x" + String::toHexString ((pointer_sized_int)data.objectValue);
    }
    ...
    static bool
    bufferEquals (const ValueUnion& data, const ValueUnion& otherData, const VariantType& otherType) noexcept
    {
        return otherType.toObject (otherData) == data.objectValue;
    }
    ...
    static bool
    binaryEquals (const ValueUnion& data, const ValueUnion& otherData, const VariantType& otherType) noexcept
    {
    ...
    {
        dest.methodValue = new NativeFunction (*source.methodValue);
    }
    ...
    static bool
    methodEquals (const ValueUnion& data, const ValueUnion& otherData, const VariantType& otherType) noexcept
    {
...
var::var (uint32 v) noexcept : type (&Instance::attributesInt64)
{
    value.int64Value = v;
}
var::var (const int64 v) noexcept : type (&Instance::attributesInt64)
{
    value.int64Value = v;
}
var::var (const bool v) noexcept : type (&Instance::attributesBool)
{
    value.boolValue = v;
}
var::var (const double v) noexcept : type (&Instance::attributesDouble)
{
    value.doubleValue = v;
}
var::var (NativeFunction m) noexcept : type (&Instance::attributesMethod)
{
    value.methodValue = new NativeFunction (m);
}
var::var (const Array<var>& v) : type (&Instance::attributesArray)
{
    value.objectValue = new VariantType::RefCountedArray (v);
}
var::var (const String& v) : type (&Instance::attributesString)
{
    new (value.stringValue) String (v);
}
var::var (const char* const v) : type (&Instance::attributesString)
{
    new (value.stringValue) String (v);
}
var::var (const wchar_t* const v) : type (&Instance::attributesString)
{
    new (value.stringValue) String (v);
}
var::var (const void* v, size_t sz) : type (&Instance::attributesBinary)
{
    value.binaryValue = new MemoryBlock (v, sz);
}
var::var (const MemoryBlock& v) : type (&Instance::attributesBinary)
{
...
var var::undefined () noexcept
{
    return var (Instance::attributesUndefined);
}
...
bool var::isBuffer () const noexcept
{
...
VariantBuffer* var::getBuffer () const noexcept
{
    ...
    if (isBuffer ())
        return (VariantBuffer*)getObject ();
    else
        return nullptr;
}
...
var& var::operator= (VariantBuffer* buffer)
{
    var v2 (buffer);
    swapWith (v2);
    return *this;
...
var::var (var&& other) noexcept : type (other.type), value (other.value)
{
...
var var::call (
    const Identifier& method, const var& arg1, const var& arg2, const var& arg3, const var& arg4, const var& arg5
) const
{
