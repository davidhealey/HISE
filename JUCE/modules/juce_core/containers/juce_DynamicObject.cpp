...
{
    auto result = new DynamicObject(*this);
...
void DynamicObject::writeAsJSON (OutputStream& out, int indentLevel, bool allOnOneLine, int maximumDecimalPlaces) 
{
    auto o = JSON::FormatOptions().withSpacing(allOnOneLine ? JSON::Spacing::singleLine : JSON::Spacing::multiLine)
                                  .withMaxDecimalPlaces(maximumDecimalPlaces)
                                  .withIndentLevel(indentLevel);

    return writeAsJSON(out, o);
}
