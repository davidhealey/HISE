...
void CompileExporter::ProjectTemplateHelpers::handleCompilerInfo(CompileExporter* exporter, String& templateProject)
{
	...
	if (exporter->useIpp)
	{
		ippRoot = exporter->getIppRoot(File::getSpecialLocation(File::userHomeDirectory));	
	
		if (ippRoot.isDirectory())
		{
			ippInclude = ippRoot.getFullPathName() + "/ipp/latest/include";
			ippLib = ippRoot.getFullPathName() + "/ipp/latest/lib";
		}	
	}
  
	REPLACE_WILDCARD_WITH_STRING("%IPP_COMPILER_FLAGS%", exporter->useIpp ? ippLib + "/libippcv.a " + ippLib + "/libippi.a " + ippLib + "/libipps.a " + ippLib + "/libippvm.a " + ippLib + "/libippcore.a" : String());
	REPLACE_WILDCARD_WITH_STRING("%IPP_HEADER%", exporter->useIpp ? ippInclude : String());
	REPLACE_WILDCARD_WITH_STRING("%IPP_LIBRARY%", exporter->useIpp ? ippLib : String());
#endif
...
File CompileExporter::getIppRoot(File fallbackStartDir)
{
	// Common IPP root paths to try first
	const Array<File> knownRoots = {
		File("/opt/intel/oneapi"),
		File(File::getSpecialLocation(File::userHomeDirectory).getChildFile("intel/oneapi")),
		File(File::getSpecialLocation(File::userHomeDirectory).getChildFile(".local/share/intel/oneapi"))
	};

	for (const auto& root : knownRoots)
	{
			if (root.exists() && root.getChildFile("ipp").exists())
				return root;
	}

	RangedDirectoryIterator iter(fallbackStartDir, true, "*.ipp.h", File::findFiles);

	for (auto result : iter)
	{
			auto found = result.getFile();
			auto path = found.getFullPathName();

			if (path.containsIgnoreCase("/ipp/") && path.containsIgnoreCase("/include/"))
					return found.getParentDirectory().getParentDirectory().getParentDirectory();
	}
