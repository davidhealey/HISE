...
void CompileExporter::ProjectTemplateHelpers::handleCompilerInfo(CompileExporter* exporter, String& templateProject)
{
	...
	if (hasMoonbase)
	{
		...
		const bool isStandalone = templateProject.contains("IS_STANDALONE_APP=\"enabled\"");
		REPLACE_WILDCARD_WITH_STRING("%MOONBASE_APP_MODE%", isStandalone ? "enabled" : "disabled");
	}
	else
	{
		REPLACE_WILDCARD_WITH_STRING("%MOONBASE_MODULEINFO%", "");
		REPLACE_WILDCARD_WITH_STRING("%MOONBASE_APP_MODE%", "disabled");
	}
...
void CompileExporter::BatchFileCreator::createBatchFile(CompileExporter* exporter, BuildOption buildOption, TargetTypes types, ChildProcessManager* manager)
{
	...
	// Moonbase prebuild: run PreBuild.sh before Projucer --resave if MOONBASE is active
	const File moonbasePreBuildScript = exporter->hisePath.getChildFile("moonbase_JUCEClient/PreBuild.sh");
	const File moonbaseApiConfig = GET_PROJECT_HANDLER(chainToExport).getRootFolder().getChildFile("moonbase_api_config.json");

	const String allDefines = exporter->dataObject.getSetting(HiseSettings::Project::ExtraDefinitionsWindows).toString()
	...
	ADD_LINE("");
	if (addMoonbasePreBuild)
	{
		// Convert Windows paths (C:\...) to WSL mount paths (/mnt/c/...) for use with wsl bash
		auto toWslPath = [](const String& winPath) -> String
		{
			if (winPath.length() >= 2 && winPath[1] == ':')
				return "/mnt/" + winPath.substring(0, 1).toLowerCase() + winPath.substring(2).replace("\\", "/");
			return winPath.replace("\\", "/");
		};

		const String winPreBuildPath = moonbasePreBuildScript.getFullPathName();
		const String winConfigPath   = moonbaseApiConfig.getFullPathName();

		// Try Git for Windows first (most common on Windows), then WSL, then error
		ADD_LINE("if exist \"C:\\Program Files\\Git\\bin\\bash.exe\" (");
		ADD_LINE("    \"C:\\Program Files\\Git\\bin\\bash.exe\" \"" << winPreBuildPath << "\" \"" << winConfigPath << "\"");
		ADD_LINE(") else (");
		ADD_LINE("    where wsl 1>nul 2>nul");
		ADD_LINE("    if errorlevel 1 (");
		ADD_LINE("        echo ERROR: No bash interpreter found. Install Git for Windows ^(git-scm.com^) or WSL.");
		ADD_LINE("        exit /b 1");
		ADD_LINE("    )");
		ADD_LINE("    wsl bash \"" << toWslPath(winPreBuildPath) << "\" \"" << toWslPath(winConfigPath) << "\"");
		ADD_LINE(")");
	}
	...
	if (addMoonbasePreBuild)
		ADD_LINE("bash \"" << moonbasePreBuildScript.getFullPathName() << "\" \"" << moonbaseApiConfig.getFullPathName() << "\"");
