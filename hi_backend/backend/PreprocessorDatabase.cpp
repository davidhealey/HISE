...
PreprocessorDataBase::PreprocessorDataBase()
{
	...
	data["HISE_USE_EXTERNAL_AUTOMATION_DATA"] = Entry()
		.withCategory(Category::PresetAndState)
		...
		.withDescriptionLine("When enabled, the assignments (MIDI CC learn mappings, macro connections and MPE modulator connections) are no longer written into the preset or the DAW project state. Instead they are kept in a 'AutomationData.json' file that sits in the root folder of the currently active file handler, so full instrument expansions each get their own assignment file. Whenever a preset is changed, a DAW session is loaded or the plugin / expansion is loaded, the assignments are restored from that file. As soon as the user changes an assignment, the file is created (and kept up to date from then on).")
		.withDescriptionLine("If no assignment file exists yet, HISE falls back to the default behaviour of using the values stored in the preset. The feature can be toggled at runtime with Engine.setUseExternalAutomationData().")
		.withDefault(0)
		.withValue(HISE_USE_EXTERNAL_AUTOMATION_DATA)
		.withCrossReference(LinkType::ScriptingApi, "Engine.setUseExternalAutomationData", "enables or disables the feature at runtime");
