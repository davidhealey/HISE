	...
	defaultFont(GLOBAL_FONT().getTypefacePtr(), "Oxygen")
{
	...
	externalAssignmentHandler = new ExternalControllerAssignmentHandler(this);
#endif
...
#if HISE_USE_EXTERNAL_ASSIGNMENT_FILE
	// Destroy this before the expansion handler goes away (it deregisters itself as a listener).
	externalAssignmentHandler = nullptr;
#endif
...
#if HISE_USE_EXTERNAL_ASSIGNMENT_FILE
			// Make sure the assignments stored in the external file are applied when the project is loaded.
...
void MainController::savePluginState(MemoryBlock& destData, int currentlyLoadedProgram)
{
	...
	// When the external assignment file is active, the MIDI / MPE / macro assignments live in that
	// file rather than in the plugin state (DAW session).
